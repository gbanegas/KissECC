...
#        return t
    ...
    def edwards(self, P, Q):
        ...
        x2 = Q[0]
        y2 = Q[1]
        x3 = (x1*y2+x2*y1) * inv(1+self.b*x1*x2*y1*y2)
        y3 = (y1*y2+x1*x2) * inv(1-self.b*x1*x2*y1*y2)
        return Coord(x3,y3)

    def scalar_multiplication(self, P,e):
        print "Starting with e: ", e
        if e == 0: return self.zero
        print "Scalar with e: ", e
        Q = self.scalar_multiplication(P,e/2)
        print "edwards with e: ", e
        Q = self.edwards(Q,Q)
        print "edwards with e: ", e
