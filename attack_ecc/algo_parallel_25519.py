...
def generate_v_values(d, N):
    for i in xrange(0, N):
        value = d + (alpha[i]*q)
        v.append(value)

def  generate_alpha_js(N):
    for i in xrange(1, N+1):
        al = r[i] - r[0]
        alpha.append(int(math.fabs(al)))

def  generate_r_js(n, N):
    for i in xrange(0, N+1):
        a = random.getrandbits(n)
        r.append(int(math.fabs(a)))

def bit_flip_random(bit_list, randomized_bits):
    bit_list_t = bit_list[:]
    pos_list = []
...
def generate_v_values_with_bit_flip(d, N, randomized_bits):
    for i in xrange(0, N):
        value = d + (alpha[i]*q)
...
def wide_widow_attack(d, window_size = 10, n = 512, N = 200, randomized_bits = 30):
    generate_r_js(n, N)
    generate_alpha_js(N)
    generate_v_values_with_bit_flip(d, N, randomized_bits)
